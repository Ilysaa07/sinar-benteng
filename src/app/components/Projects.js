...
export default function Projects() {
  const projects = [
    ...
    },
    {
      ...
      slug: 'renovasi-hotel-berbintang',
      category: 'Renovasi',
      location: 'Bali',
      year: '2024',
    },
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-slate-300 text-lg font-light">
            Pelajari bagaimana kami menyelesaikan tantangan nyata di lapangan secara presisi, tepat waktu, dan sesuai standar keamanan industri.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project, index) => (
            <Link href={`/proyek/${project.slug}`} key={project.slug || index} className="block">
              <div
                className="group relative overflow-hidden bg-[#1e293b]/50 backdrop-blur-sm shadow-2xl transition-all duration-300 rounded-sm border border-white/5 hover:border-[#e8bf56]/50 h-full cursor-pointer"
              >
                ...
                {/* Content Overlay */}
                <div className="absolute bottom-0 left-0 right-0 bg-[#0f172a]/95 border-t-2 border-[#e8bf56] p-6 transition-transform duration-300 group-hover:-translate-y-2">
                  <div className="mb-2 flex justify-between items-center">
                    <span className="bg-[#e8bf56] text-white text-[10px] font-bold px-2 py-0.5 uppercase tracking-wide rounded-sm">
                      {project.category}
                    </span>
                    <span className="text-[#e8bf56] text-xs font-bold uppercase flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      Lihat Studi Kasus
                  ...
                  <div className="grid grid-cols-2 gap-4 text-slate-400 text-xs uppercase tracking-wider mt-4 opacity-70 group-hover:opacity-100 transition-opacity">
